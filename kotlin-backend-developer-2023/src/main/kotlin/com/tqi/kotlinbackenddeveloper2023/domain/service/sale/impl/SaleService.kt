...
class SaleService(
    ...
    override fun finalizeSale(sale: Sale): ProofOfSale {
        ...
        if (shoppingCart.listProductsPlacedInTheCart.isEmpty()) {
            throw BusinessException("add a product to the cart before finalizing the sale")
        }

        sale.totalSaleAmount = shoppingCart.cartValue

        val proofOfSale = ProofOfSale(
            0L,
            LocalDateTime.now(),
            shoppingCart.cartValue,
            sale.paymentOptions.toString(),
            mutableListOf()
        )
